...
const AdminOrderDetail = () => {
  ...
  const fetchOrder = async () => {
    ...
    try {
      ...
      // Auto-verify cashfree pending payments (admins OR just run for robustness)
      if (
        o.paymentInfo &&
        o.paymentInfo.method === 'cashfree' &&
        o.paymentInfo.status === 'pending'
      ) {
        // run automatic verify but don't block rendering
        doVerifyPayment(); // fire-and-forget; it will refresh order on success
      }
    } catch (error) {
      console.error('Fetch order error:', error);
      toast.error('Failed to fetch order details');
  ...
  // Manual verify (and used by auto flow)
  const doVerifyPayment = async () => {
    ...
    try {
      ...
      if (res.data && res.data.verified) {
        ...
        // not verified yet — show gentle info (do not toast error)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="card">
            <div className="flex justify-between items-start mb-6">
              <div>
                ...
                <p className="text-sm text-gray-600 mt-1">Placed on {new Date(order.createdAt).toLocaleString('en-IN')}</p>
                {order.expectedDeliveryDate && (
                  <p className="text-sm text-emerald-700 mt-1 font-medium">
            ...
            <div>
              ...
              <div className="space-y-4">
                {Array.isArray(order.items) && order.items.map((item, index) => (
                  <div key={index} className="flex items-center border-b pb-4 last:border-b-0">
                    <img src={item.image || '/placeholder.png'} alt={item.name} className="w-20 h-20 object-contain rounded" />
                    <div className="ml-4 flex-1">
                      <h3 className="font-semibold text-gray-900">{item.name}</h3>
                      <p className="text-sm text-gray-600">Quantity: {item.quantity}</p>
                      <p className="text-sm text-gray-600">Price: ₹{(item.discountPrice || item.price).toLocaleString()}</p>
                    </div>
                    <div className="text-right">
                      <p className="font-semibold text-gray-900">₹{(((item.discountPrice || item.price) * item.quantity) || 0).toLocaleString()}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="card">
              ...
              <div className="space-y-2 text-sm">
                ...
                <p><span className="font-medium">Mobile:</span> {order.user?.mobile || ''}</p>
              </div>
            </div>
            ...
            <div className="card">
              ...
              <div className="text-sm text-gray-700">
                ...
                <p>{order.shippingAddress?.country}</p>
                <p className="mt-2"><span className="font-medium">Mobile:</span> {order.shippingAddress?.mobile}</p>
              </div>
            </div>
          </div>
          ...
          <div className="card">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="font-medium">Payment Method:</span>
                <span className="uppercase">{order.paymentInfo?.method || 'N/A'}</span>
              </div>
              ...
              <div className="flex justify-between">
                ...
                <span className={`px-2 py-1 rounded-full text-xs ${
                  ...
                  order.paymentInfo?.status === 'failed' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800'
                }`}>
                  {order.paymentInfo?.status || 'pending'}
                </span>
              </div>
              ...
              {order.paymentInfo?.cashfreeOrderId && (
                <div className="flex justify-between">
                  <span className="font-medium">Cashfree Link ID:</span>
                  <span className="font-mono text-xs">{order.paymentInfo.cashfreeOrderId}</span>
                </div>
              )}
              {order.paymentInfo?.cashfreeReferenceId && (
                <div className="flex justify-between">
                  <span className="font-medium">Transaction ID:</span>
                  <span className="font-mono text-xs">{order.paymentInfo.cashfreeReferenceId}</span>
                </div>
              )}
              {order.paymentInfo?.cashfreePaymentMode && (
                <div className="flex justify-between">
                  <span className="font-medium">Payment Mode:</span>
                  <span className="font-mono text-xs">{order.paymentInfo.cashfreePaymentMode}</span>
                </div>
              )}
              ...
              {order.paymentInfo?.paidAt && (
                <div className="flex justify-between">
                  <span className="font-medium">Paid At:</span>
                  <span>{new Date(order.paymentInfo.paidAt).toLocaleString('en-IN')}</span>
                </div>
              )}

              {/* Manual verify button for admins */}
              {hasAdminAccess && order.paymentInfo?.method === 'cashfree' && order.paymentInfo?.status === 'pending' && (
                <div className="mt-3">
                  ...
                  >
                    {verifying ? 'Verifying…' : <><RefreshCw size={14}/> Verify payment</>}
                  </button>
                </div>
              )}
            </div>
          ...
          <div className="card">
            ...
            <div className="space-y-4">
              {order.statusHistory && order.statusHistory.length > 0 ? (
                order.statusHistory.map((history, index) => (
                  <div key={index} className="flex items-start">
                    <div className={`p-2 rounded-full ${getStatusColor(history.status)}`}>{getStatusIcon(history.status)}</div>
                    <div className="ml-4">
                      <p className="font-medium capitalize">{history.status}</p>
                      <p className="text-sm text-gray-600">{new Date(history.changedAt || history.date).toLocaleString('en-IN')}</p>
        ...
        <div className="space-y-6">
          ...
          <div className="card">
            <h2 className="text-lg font-semibold mb-4">Order Summary</h2>
            <div className="space-y-3 text-sm">
              <div className="flex justify-between"><span className="text-gray-600">Subtotal</span><span className="font-semibold">₹{(order.itemsPrice || 0).toLocaleString()}</span></div>
              <div className="flex justify-between"><span className="text-gray-600">Tax</span><span className="font-semibold">₹{(order.taxPrice || 0).toLocaleString()}</span></div>
              <div className="flex justify-between"><span className="text-gray-600">Shipping</span><span className="font-semibold">{order.shippingPrice === 0 ? <span className="text-green-600">FREE</span> : `₹${(order.shippingPrice || 0).toLocaleString()}`}</span></div>
              <div className="border-t pt-3 flex justify-between"><span className="font-semibold text-base">Total</span><span className="font-bold text-lg text-primary-600">₹{(order.totalPrice || 0).toLocaleString()}</span></div>
            </div>
          </div>
          ...
          {(order.carrier || order.trackingId || order.trackingUrl) && (
            <div className="card">
              ...
              <div className="space-y-2 text-sm">
                {order.carrier && <div className="flex justify-between"><span className="text-gray-600">Courier</span><span className="font-semibold">{order.carrier}</span></div>}
                {order.trackingId && <div className="flex justify-between"><span className="text-gray-600">Tracking ID</span><span className="font-mono text-xs">{order.trackingId}</span></div>}
                {order.trackingUrl && (
                  <div className="flex items-center justify-between mt-2">
                    <span className="text-gray-600 text-sm">Tracking link</span>
                    <a href={order.trackingUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center text-primary-600 hover:text-primary-700 text-xs font-medium">
          ...
          {hasAdminAccess && order.paymentInfo?.method === 'cod' && order.paymentInfo?.status === 'pending' && (
            <div className="card bg-yellow-50 border border-yellow-200">
              <h2 className="text-lg font-semibold mb-4 text-yellow-800">Cash on Delivery - Payment Pending</h2>
              <p className="text-sm text-gray-700 mb-4">This order is COD. Mark payment as completed when cash is collected during delivery.</p>
              <button onClick={handleMarkCODCompleted} className="btn bg-green-600 text-white hover:bg-green-700">Mark Payment as Collected</button>
            </div>
          )}
          ...
          {hasAdminAccess && (
            <div className="card">
              <h2 className="text-lg font-semibold mb-2">Update Status & Tracking</h2>

              {isTerminal && <p className="mb-3 text-xs text-gray-500">This order is <strong>{currentStatus}</strong>. Status and tracking are locked and cannot be changed.</p>}

              <form onSubmit={handleStatusUpdate} className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Status</label>
                  <select value={statusUpdate.status} onChange={(e) => setStatusUpdate((prev) => ({ ...prev, status: e.target.value }))} className="input" required disabled={!canEditStatus}>
                  ...
                  <p className="mt-1 text-xs text-gray-500">Once you move to a later step (e.g. shipped), earlier statuses (pending/processing) are disabled.</p>
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Expected delivery date</label>
                  <input type="date" value={statusUpdate.expectedDeliveryDate} onChange={(e) => setStatusUpdate((prev) => ({ ...prev, expectedDeliveryDate: e.target.value }))} className="input" disabled={!canEditStatus} />
                  <p className="mt-1 text-xs text-gray-500">Leave empty when marking as "shipped" to let backend auto-calc using DEFAULT_DELIVERY_DAYS.</p>
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Courier name</label>
                  <input type="text" value={statusUpdate.carrier} onChange={(e) => setStatusUpdate((prev) => ({ ...prev, carrier: e.target.value }))} className="input" placeholder="e.g. Delhivery, DTDC" disabled={!canEditStatus} />
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Tracking ID</label>
                  <input type="text" value={statusUpdate.trackingId} onChange={(e) => setStatusUpdate((prev) => ({ ...prev, trackingId: e.target.value }))} className="input" placeholder="AWB / tracking number" disabled={!canEditStatus} />
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Tracking URL</label>
                  <input type="url" value={statusUpdate.trackingUrl} onChange={(e) => setStatusUpdate((prev) => ({ ...prev, trackingUrl: e.target.value }))} className="input" placeholder="https://courier.com/track/..." disabled={!canEditStatus} />
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Note (Optional)</label>
                  <textarea value={statusUpdate.note} onChange={(e) => setStatusUpdate((prev) => ({ ...prev, note: e.target.value }))} rows="3" className="input" placeholder="Add a note about this update (visible in timeline)..." disabled={!canEditStatus} />
                </div>

                <button type="submit" disabled={updating || !canEditStatus} className="w-full btn btn-primary disabled:opacity-50">
                  {updating ? 'Updating...' : 'Save Status & Tracking'}
                </button>
              </form>
            </div>
